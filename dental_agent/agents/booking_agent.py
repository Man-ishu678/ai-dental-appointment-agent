...
from dental_agent.config.settings import GROQ_API_KEY,TEMPERATURE, MODEL_NAME
...
- If ANY required parameter is missing (patient_id, specialization, doctor_name, date_slot),DO NOT call any tool.
- Instead ask the user for the missing information.
- Only call tools when all parameters are known.
